...
if os.path.exists("0.7.1"):
    for file in glob.glob(os.path.join("0.7.1", "pyunity*.whl")):
        os.remove(file)
else:
    os.mkdir("0.7.1")
...
for platform, plat_name in zip(
    ...
    for version, name in zip(
        ...
        #print("https://ci.appveyor.com/api/projects/pyunity/pyunity/artifacts/dist/pyunity-0.7.1-" + name + plat_name + ".whl?job=" + job_name)
        try:
            wget.download("https://ci.appveyor.com/api/projects/pyunity/pyunity/artifacts/dist/pyunity-0.7.1-" + name + plat_name + ".whl?job=" + job_name, "0.7.1/pyunity-0.7.1-" + name + plat_name + ".whl")
        except urllib.error.HTTPError:
            print("Couldnt download pyunity-0.7.1-" + name + plat_name + ".whl", end="")
        print()

linux_builds = glob.glob("0.7.1/pyunity-0.7.1*linux*.whl")
