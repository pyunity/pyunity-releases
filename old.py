...
for platform, plat_name in zip(
    ...
    for version, name in zip(
        ...
        try:
            wget.download("https://ci.appveyor.com/api/projects/pyunity/pyunity/artifacts/dist/pyunity-0.9.0-" + name + plat_name + ".whl?job=" + job_name, "post-0.4/0.9.0/pyunity-0.9.0-" + name + plat_name + ".whl")
        except urllib.error.HTTPError:
            print("Couldnt download pyunity-0.9.0-" + name + plat_name + ".whl", end="")
        print()

linux_builds = glob.glob("post-0.4/0.9.0/pyunity-0.9.0*linux*.whl")
